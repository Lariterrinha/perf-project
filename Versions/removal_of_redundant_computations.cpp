...
void my_sobel(Mat im_in, Mat im_out) {
    ...
    for (int j = 1; j < rows-1; j++) {
        ...
        // loop without branch inside: process until i = cols-3, where i+2 is valid
        int last_safe = cols - 3;
        int i = 1;
        for (; i <= last_safe; ++i) {
            ...
            // advance: read next column (i+2) which is safe here
        ...
        // handle remaining columns (i = last_safe+1 .. cols-2) using safe indexed loads
        for (; i < cols-1; ++i) {
